...
def build_matrix_M(N, h):
    ...
    for i in range(N * N):
        ...
        M[i, i] = 4 / h ** 2
        # sous-diagonale en excluant le bord de gauche
        if i % N != 0:
            M[i, i - 1] = -1 / h ** 2
        # sur-diagonale en excluant le bord de droite
        if (i + 1) % N != 0:
            M[i, i + 1] = -1 / h ** 2
        # deuxieme sous-diagonale en excluant le bord inférieur
        if i >= N:
            M[i, i - N] = -1 / h ** 2
        # deuxième sur-diagonale en excluant le bord supérieur
        if i < N * (N - 1):
            M[i, i + N] = -1 / h ** 2
...
def solve_laplace_eq(N, n, k, a, b):
    ...
    # Coordonnées des points de discrétisation
    ...
    # Calcul des valeurs du terme source f
    for i in range(N):
        for j in range(N):
            x = xi[i]
            y = yj[j]
